export const calculateHostStatus = (host, tipoTerminal) => {
  ...
  if (tipoTerminal === 'wst') {
    // 2. Buscar el último wst_ipa_v que se aplicó correctamente 
    ...
    // 4. Verificar si hay un wst_upd_v1.8.1 fallido en los trabajos relevantes
    const hasFailedUpdate = relevantSummaries.some(
      summary => summary.job_name === 'wst_upd_v1.8.1' && summary.failed
    );

    // 5. Verificar si hay un wst_upd_v1.8.1 exitoso en los trabajos relevantes
    const hasSuccessfulUpdate = relevantSummaries.some(
      summary => summary.job_name === 'wst_upd_v1.8.1' && !summary.failed
    );

    // Regla 2: Si hay un wst_ipa_v exitoso y un wst_upd_v1.8.1 fallido después, estado es fallido
    if (hasSuccessfulIPA && hasFailedUpdate) {
      return 'fallido';
    }

    // Regla 4: Si no hay wst_ipa_v exitoso y hay un wst_upd_v1.8.1 exitoso, estado es actualizado
    if (!hasSuccessfulIPA && hasSuccessfulUpdate) {
      return 'actualizado';
    }

    // Regla 1: Si hay un wst_ipa_v exitoso y un wst_upd_v1.8.1 exitoso después, estado es actualizado
    if (hasSuccessfulIPA && hasSuccessfulUpdate) {
      return 'actualizado';
    }

    // Regla 3: Si hay un wst_ipa_v exitoso y no hay un wst_upd_v1.8.1 posterior, estado es pendiente
    if (hasSuccessfulIPA) {
      return 'pendiente';
    }
